                         ...
                         MPI_Comm cart_comm)
{
    ...
    if (dims[0] > 1)
    {
        ...
        for (int j = 0; j < local_nj + 2; ++j)
        {
            for (int k = 0; k < local_nk + 2; ++k)
            {
                send_buffer_left[j*(local_nk + 2) + k] = u[local_index(1, j, k)];
                send_buffer_right[j*(local_nk + 2) + k] = u[local_index(local_ni, j, k)];
            }
        }
    ...
    if (dims[1] > 1)
    {
        ...
        for (int i = 0; i < local_ni + 2; ++i)
        {
            for (int k = 0; k < local_nk + 2; ++k)
            {
                send_buffer_down[i*(local_nk + 2) + k] = u[local_index(i, 1, k)];
                send_buffer_up[i*(local_nk + 2) + k] = u[local_index(i, local_nj, k)];
            }
        }
    ...
    if (dims[2] > 1)
    {
        ...
        for (int i = 0; i < local_ni + 2; ++i)
        {
            for (int j = 0; j < local_nj + 2; ++j)
            {
                send_buffer_back[i*(local_nj + 2) + j] = u[local_index(i, j, 1)];
                send_buffer_front[i*(local_nj + 2) + j] = u[local_index(i, j, local_nk)];
            }
        }
    ...
    if (dims[0] > 1)
    {
        ...
        for (int j = 0; j < local_nj + 2; ++j)
        {
            for (int k = 0; k < local_nk + 2; ++k)
            {
                u[local_index(local_ni + 1, j, k)] = recv_buffer_right[j*(local_nk + 2) + k];
                u[local_index(0, j, k)] = recv_buffer_left[j * (local_nk + 2) + k];
            }
        }
    }
    ...
    if (dims[1] > 1)
    {
        ...
        for (int i = 0; i < local_ni + 2; ++i)
        {
            for (int k = 0; k < local_nk + 2; ++k)
            {
                u[local_index(i, local_nj + 1, k)] = recv_buffer_up[i*(local_nk + 2) + k];
                u[local_index(i, 0, k)] = recv_buffer_down[i*(local_nk + 2) + k];
                ++idx;
            }
        }
    }
    ...
    if (dims[2] > 1)
    {
        ...
        for (int i = 0; i < local_ni + 2; ++i)
        {
            for (int j = 0; j < local_nj + 2; ++j)
            {
                u[local_index(i, j, local_nk + 1)] = recv_buffer_front[i*(local_nj + 2) + j];
                u[local_index(i, j, 0)] = recv_buffer_back[i*(local_nj + 2) + j];
            }
        }
    }
}
